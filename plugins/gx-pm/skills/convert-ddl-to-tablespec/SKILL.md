...
**입력항목의 제약이 `[가정]` 이면 그 표식을 `근거` 열에 이어 적는다.**

```
행안부표준 · 권한=AUTHRT, 명=NM · [가정] 길이 100 — RFP 미규정
```

`길이` 열에는 숫자만 남으므로, 여기서 표식을 떨어뜨리면 **DE-13 이 무표식 값에서
경계를 뽑는다.** 제약의 최종 승자가 DE-08 이라 그 뒤로는 되살릴 자리가 없다
(`skills/generate-unit-test-plan/SKILL.md` Step 2). 표식 자리의 정본은
`templates/evidence-rules.md` §표식은 세 홉을 관통한다 다.

`[미확정]` 제약이면 그 컬럼의 `길이` 를 비우고 `근거` 에 사유를 적는다 — 값을
지어내지 않는다.

제약 불일치는 DE-08 의 열에 남기지 않는다 — 15컬럼 정본에 그 자리가 없다.
대신 버리지 않고 Step 6 승인 게이트의 `[제약 불일치]` 블록으로 한 번 보고해 사용자 판단을 받는다.
`입력항목 50자 ↔ 컬럼 VARCHAR(100)` 은 둘 중 하나가 틀렸다는 뜻이다.

### Step 3: 데이터 요구사항 반영

AN-02 에는 기능을 거치지 않고 데이터의 구성·규칙·제약만 규정하는 요구사항이 있다
(회차 누적, 이력 보존, 코드 체계처럼 여러 기능에 걸쳐 적용되는 설계 제약). 이런
요구사항은 AN-03 의 어느 `연계요구사항ID` 에도 걸리지 않아 위 컬럼 매핑에서
그냥 지나치기 쉽다 — 하지만 스키마 설계에는 반드시 반영해야 하는 제약이다.

AN-02 요구사항 중 AN-03 의 어느 기능에도 안 걸리면서, 상세내용이 시스템의 동작이
아니라 데이터 자체의 구성·범위·누적·보존 규칙을 규정하는 것을 **데이터 요구사항**
으로 본다. 이 절이 그 정의의 정본이다.

| 처리 | 방법 |
|------|------|
| 컬럼에 반영 가능 | 그 컬럼(들)의 `근거` 열에 요구사항ID 를 덧붙인다: `행안부표준 · 회차=RND · DAR-005 회차 누적 규칙 반영` |
| 반영 불가·판단 필요 | 어느 컬럼에 반영할지 특정할 수 없거나 스키마 변경이 필요하면 **차단하지 않고** Step 6 승인 게이트에 `[설계 제약 미반영]` 목록으로 올린다 |

이 단계를 건너뛰면 이 요구사항의 흔적이 DE-08 어디에도 남지 않는다.
`templates/AN-05-traceability-matrix.md` 의 「설계 제약 미반영」 판정이 바로 그 흔적의
유무를 본다.
...
[설계 제약 미반영]  데이터 요구사항인데 반영할 컬럼을 특정하지 못했습니다.
  DAR-005  회차 누적 규칙 — 어느 컬럼에 반영할지 판단이 필요합니다
```
