...
패키지 구조에서 추론한 업무 영역 분류를 마크다운 표로 출력한 후, **AskUserQuestion 도구**로 승인을 요청한다:

```
소스코드에서 업무 영역을 식별했습니다. 분류가 맞는지 확인해주세요.
...
화면이 없는 프로그램 목록을 표로 출력한 후, **AskUserQuestion 도구**로 포함 여부를 질문한다:

```
화면이 없는 프로그램 {N}건이 발견되었습니다. 프로그램정의서에 포함할까요?
(포함/제외/일부 선택)
```
