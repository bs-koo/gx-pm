...
넷 다 실패했거나 길이·범위·형식·열거값을 못 찾았으면 **2단 판정**으로 간다 —
RFP 가 그 값을 규정하지 않았고 없어도 구현되면 `[가정]`, RFP 가 규정했는데 값이 없어
그것 없이는 구현이 안 되면 `[미확정]` 이다. **`[미확정]` 은 지어내지 않는다.**
`이메일(필수)` 는 제약이 빈 것이고, `이메일(필수, 1~255자)` 는 찬 것이다.
판정 기준은 `templates/evidence-rules.md` §[가정] 과 [미확정] 은 2단 판정으로 갈린다 가
정본이다. `[가정]` 표식이 DE-08·DE-13 까지 이어지는 규칙은 같은 문서 §표식은 세 홉을
관통한다 를 따른다.
...
|---|---|---|---|---|---|---|---|---|---|
| FN-001 | 데이터 전처리·입력 | 상대평가기준 생성 기능 | 벤치마크 스코어테이블 생성 | 원단위 통계 데이터로 세부용도별 EUI 테이블과 CDF 를 산출한다 | 원단위 통계파일(xlsx, 필수) / 세부용도 코드(필수) / 이상치 제외 여부(Y·N, 기본 N) | ① 헤더 검증, 형식이 다르면 오류를 반환하고 중단한다 ② 세부용도별 그룹핑, 이상치 제외 여부가 Y 이면 이상치를 제외한다 ③ EUI 중위값·분위수 산출 ④ CDF 계산 ⑤ 스코어테이블 저장·커밋, 실패 시 롤백 | 생성 완료 안내 / TB_SCORE 1건 저장 | REQ-001 | |
