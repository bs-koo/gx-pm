...
DOCUMENT_PROFILES = {
    ...
    "추적매트릭스": {
        ...
        # 컬럼 세트의 정본은 templates/AN-05-traceability-matrix.md 의
        # 「본문 표 헤더 (플랫)」 절이다. 여기서 이름을 바꾸면 계약 테스트가 잡는다.
        # 전체 세트와 축약 세트를 둘 다 둔다 — 하나로 두면 컬럼이 적은 파일이
        # _MATCH_THRESHOLD 아래로 떨어져 재배열이 조용히 멈춘다.
        "columns": [
            # 전체 세트 (29) — 템플릿 플랫 헤더 순서 그대로
            ["제안요청ID", "제안요청 내역", "대분류", "중분류", "소분류", "요구사항ID",
             "요구사항명", "요구사항 내역", "해결/구현방안", "제약사항", "구분", "수용여부", "근거",
             "인터페이스ID", "인터페이스명", "테이블ID", "화면ID", "화면명", "프로그램ID",
             "프로그램명", "단위테스트ID", "테스트케이스 수", "통합테스트ID", "통합테스트시나리오명",
             "시스템테스트ID", "시스템테스트 항목", "잔존 결함", "과업완료여부", "검사결과"],
            # 축약 세트 (23) — 실무에서 흔한 형태.
            # 전체 세트에서 대/중/소분류·요구사항명·해결방안·제약사항을 뺀 것
            ["제안요청ID", "제안요청 내역", "요구사항ID", "요구사항 내역", "구분", "수용여부",
             "근거", "인터페이스ID", "인터페이스명", "테이블ID", "화면ID", "화면명",
             "프로그램ID", "프로그램명", "단위테스트ID", "테스트케이스 수", "통합테스트ID",
