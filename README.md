...
`/gx-명세일괄` 하나로 요구사항 → 기능 → 테이블 → 단위테스트 → 추적매트릭스를 순서대로 만들며, 판단이 필요한 **승인 게이트 3곳**에서만 멈춥니다.
...
4. **gx-pm** 탭 선택 → **설치**
5. 프로젝트에서 `/gx-프로젝트설정` → `/gx-명세일괄` 순으로 사용
...
| 이렇게 말하면 | 커맨드 | 결과물 |
|--------------|--------|--------|
| "명세 다 만들어줘" | `/gx-명세일괄` | **명세 5종 일괄 (승인 게이트 3곳)** |
...
### 6. 확인요청서 — 미결을 엑셀로 주고받기

RFP 가 값을 안 준 항목은 AI 가 임의로 정할 수 없습니다. 그렇다고 세션 안에서
스무 번 물어보면 피곤하고 토큰도 많이 씁니다. **엑셀 한 장으로 주고받습니다.**

| 미결 합계 | 어떻게 |
|---|---|
| 0건 | 물을 것이 없습니다 |
| 1~5건 | 게이트에서 **그 자리에 대화로** 묻습니다 |
| 6건 이상 | **확인요청서를 내고 멈춥니다.** 최대 3차 왕복 |

확인요청서는 시트가 넷입니다 — 안내 · 가정 확인 · 미확정 확인 · 요청 이력.
채울 칸은 **연노랑 배경에 드롭다운**입니다. 한 줄도 안 채워도 다음 차수로
넘어갑니다.

**값 대신 질문을 적어도 됩니다.** 「왜 이게 필요한지 설명해줘」라고 쓰면 다음
차수에 답이 실려 돌아옵니다. 대화로 답하지 않으므로 주고받은 것이 파일에 다
남습니다.

**「요청 이력」 시트가 감리 증거입니다.** 무엇을 물었고 · AI 는 무엇을 제시했고 ·
몇 번 물었고 · 무엇을 받았고 · 어떻게 끝났는지가 남습니다.

### 7. 미확정이 0이어야 최종본이 나갑니다

`[미확정]` 이 1건이라도 남아 있으면 **xlsx 를 뽑지 않습니다.** 마크다운 5종은
게이트마다 저장되니 작업은 안 막힙니다. 감리와 개발자에게 나가는 최종본에만
도장을 안 찍는 것입니다.

**값을 대신 정하지 않기 때문입니다.** 채우면 해소된 것처럼 보이지만 실은 우리가
정한 값이라, 개발자가 그것을 확정값으로 읽고 착수합니다.

### 8. 낡은 산출물 감지

산출물은 낱개로도 만들 수 있어서, 요구사항정의서만 고치면 나머지가 옛 값으로
남습니다. **ID 는 그대로라 추적매트릭스가 초록입니다** — 끊긴 게 아니라 낡은
것이라 누락 판정에 안 걸립니다.

그래서 머리말에 근거 버전을 적습니다 (`> 근거: AN-02 v2.0`). 상위가 v3.0 이
되면 어느 커맨드를 부르든 이렇게 알립니다.
...
```
 /gx-프로젝트설정 (A유형)
       ↓
 /gx-명세일괄       (명세 5종 일괄, 승인 게이트 3곳)
       ↓
 /gx-추적매트릭스 (누락 탐지)
```

**미결이 6건 이상이면 게이트 2 에서 멈춥니다.** 확인요청서 엑셀이 나오고,
채운 뒤 `/gx-명세일괄` 을 다시 부르면 「확인요청서 반영하고 이어가기」가
선택지에 뜹니다. 최대 3차까지 왕복하며, 실행이 이렇게 나뉩니다.

| 회차 | 하는 일 | 끝난 뒤 |
|---|---|---|
| 1 | 게이트 1·2 | 1차 발행 → 중단 |
| 2 | 1차 반영 → DE-13·AN-05 → 게이트 3 | 2차 발행 → 중단 |
| 3 | 2차 반영 → 파급 처리 | 3차 발행 → 중단 |
| 4 | 3차 반영 | 미확정 0이면 완료 |

**미확정이 0이 되면 그 자리에서 끝납니다.** 「최대 3차」이지 「반드시 3차」가
아닙니다.
...
```
 /gx-프로젝트설정 (C유형 + 소스경로 + DDL)
       ↓
 /gx-명세일괄       DE-08은 DDL에서 역생성, 나머지는 기존 산출물·RFP로 정비
       ↓
 /gx-추적매트릭스 (누락 탐지)
```
...
| `trace-requirements` | AN-05 추적매트릭스 생성/갱신. 누락 8유형 탐지 |
...
| `manage-revision-history` | 5종 공통 개정이력 행 관리. 직전 버전과 대조해 초안을 만들고 승인을 받음. **근거 버전 표기도 갱신** |
| `apply-confirmations` | 확인요청서에 사람이 적은 응답을 읽어 산출물에 반영하고 파급을 처리. 되묻기는 다음 차수 행으로 |
