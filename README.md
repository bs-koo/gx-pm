...
[![Commands](https://img.shields.io/badge/commands-14-orange.svg)]()
...
**v1.3.0**: 개발 완료 후 산출물을 맞춰야 할 때, 소스코드와 DB에서 산출물을 **역생성**할 수 있습니다. `/프로젝트설정` 한 번이면 모든 커맨드가 프로젝트 상황에 맞게 자동 동작합니다.
...
| "외부 연동 정의해줘" | `/인터페이스정의서` | DE-04 인터페이스정의서 |
| "DDL로 테이블정의서 만들어줘" | `/테이블정의서` | DE-08 테이블정의서 |
| "테스트 계획 세워줘" | `/총괄테스트계획서` | **TE-01 총괄 테스트계획서** |
| "단위테스트 계획 세워줘" | `/단위테스트계획서` | DE-13 단위테스트계획서 + **테스트케이스** |
| "통합테스트 시나리오 만들어줘" | `/통합테스트시나리오` | DE-14 통합테스트시나리오 + **예외 흐름** |
| "성능·보안 테스트 계획해줘" | `/시스템테스트` | **ST-01 / ST-02 시스템테스트 (비기능)** |
| "테스트 결과 기입해줘" | `/테스트결과서` | IM-03 / TE-02 / TE-06 결과서 |
| "결함 관리해줘" | `/결함관리대장` | **DF-01 결함관리대장** |
